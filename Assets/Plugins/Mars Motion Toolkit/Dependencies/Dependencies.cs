...
// Dependencies
// • A Dependency is a condition upon a Mars Motion feature's state, where the condition can be either to match the state or match the opposite state of that feature (for example, only while boosting, or only while not boosting). Multiple dependencies can be used in conjunction to determine whether something should happen currently. Within Mars Motion, these conditions are used for controlling the enablement of such features as locomotion movement audio and booster modules; these dependencies can be used outside of Mars Motion as well to easily tap into the state of most of the plugin's important state-based features. This can be done via providing inspector settings of Dependency arrays and using those with this class's methods, or just directly checking hardcoded Dependencies via this class's methods.
// • Dependency Requisites (Mars Motion features upon which their state may be depended) are defined by the Dependency Requisite class of scriptable objects
