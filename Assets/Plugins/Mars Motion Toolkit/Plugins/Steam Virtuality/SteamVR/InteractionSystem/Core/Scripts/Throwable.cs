...
{
	//-------------------------------------------------------------------------
	/* custom changes by Mars Motion */[RequireComponent(typeof(VelocityEstimator))]
	...
	public class Throwable : MonoBehaviour
	{
		...
		public float catchSpeedThreshold = 0.0f;/* custom changes by Mars Motion */

		[Tooltip("Whether to allow \"catching\" (attaching due to a trigger hold instead of a trigger press)")]
		public bool catchingEnabled = false;/**//* custom changes by Mars Motion */
		...
		public bool restoreOriginalParent = /* custom changes by Mars Motion */true/**/;
		...
		private void OnHandHoverBegin( Hand hand )
		{
			...
			if (/* custom changes by Mars Motion */catchingEnabled &&/**/ !attached )
			{
		...
		{
			//Trigger got pressed
			if (/* custom changes by Mars Motion */(!attached || swappingEnabled) &&/**/ hand.GetStandardInteractionButtonDown() )
			{
				hand.AttachObject( gameObject, attachmentFlags, attachmentPoint );
				ControllerButtonHints.HideButtonHint( hand, Valve.VR.EVRButtonId.k_EButton_SteamVR_Trigger );
			}
